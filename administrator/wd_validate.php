...
if($_POST['asub']=="1")
{
	$AWID = intval(scriptolution_dotcom_data($_POST['AWID']));
	$auid = intval(scriptolution_dotcom_data($_POST['auid']));
	$apay = scriptolution_dotcom_data($_POST['apay']);
	if($AWID > 0 && $auid > 0 && $apay > 0)
	{
		$sql="UPDATE members SET afunds=afunds-$apay WHERE USERID='".mysqli_real_escape_string($conn->_connectionID, $auid)."'";
		$conn->Execute($sql);
		$sql="UPDATE members SET withdrawn=withdrawn+$apay WHERE USERID='".mysqli_real_escape_string($conn->_connectionID, $auid)."'";
		$conn->Execute($sql);
		$sql="DELETE FROM withdraw_requests WHERE WID='".mysqli_real_escape_string($conn->_connectionID, $AWID)."'";
		$conn->Execute($sql);
...
if($_POST['submitform'] == "1" || ($_REQUEST['fromid']!="" || $toid>0 || $pemail!="" || $aemail!="" || $username!=""))
{
	if($fromid > 0)
	{
		$addtosql = "AND A.WID>='".mysqli_real_escape_string($conn->_connectionID, $fromid)."'";
	...
	if($toid > 0)
	{
		$addtosql .= "AND A.WID<='".mysqli_real_escape_string($conn->_connectionID, $toid)."'";
		Stemplate::assign('toid',$toid);
	}
	if($pemail != "")
	{
		$addtosql .= "AND B.pemail like'%".mysqli_real_escape_string($conn->_connectionID, $pemail)."%'";
		Stemplate::assign('pemail',$pemail);
	}
	if($aemail != "")
	{
		$addtosql .= "AND B.aemail like'%".mysqli_real_escape_string($conn->_connectionID, $aemail)."%'";
...
$query2 = "select A.*,B.username, B.afunds, B.pemail, B.aemail from withdraw_requests A, members B WHERE A.USERID=B.USERID $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
