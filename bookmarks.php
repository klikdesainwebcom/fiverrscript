...
scriptolution_dotcom_software("bookmarks");
...
$query1 = "SELECT count(*) as total from posts A, bookmarks B where A.active='1' AND B.USERID='".mysqli_real_escape_string($conn->_connectionID, $SCRIPTOLUTION_ID)."' AND A.PID=B.PID order by B.BID desc limit $config[maximum_results]";
$query2 = "SELECT A.*, B.seo, C.username, C.country from posts A, categories B, members C, bookmarks D where A.active='1' AND A.category=B.CATID AND A.USERID=C.USERID AND D.USERID='".mysqli_real_escape_string($conn->_connectionID, $SCRIPTOLUTION_ID)."' AND A.PID=D.PID order by D.BID desc limit $pagingstart, $config[items_per_page]";
